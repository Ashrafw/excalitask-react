...
const AddNewSubTask = ({
  ...
  return (
    <div
      className={`flex items-center justify-start gap-2 py-[1px] ${
        index === 0 ? "mt-2" : ""
      }`}
    >
      <button
        onClick={() => handleDeleteSubTask(subTask.id)}
        className=" flex items-center justify-center w-[30px] h-[26px] max-sm:h-[22px] text-sm  text-gray-400 p-1 rounded bg-gray-200 hover:bg-gray-300 "
      >
        <FaRegTrashAlt />
      </button>
      <input
